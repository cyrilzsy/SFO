  ...
  for (var j = 0; j < n_max; j++) {
    ...
    for (var i = 2; i < t_max; i++) {
      ...
      //   if (i > t_ordinance[k])
        ...
        z.Weekly_Consumer_Demand[j][i] = Par[4] + Par[5] * Training + Par[6] * Signage + Par[7] * Convenience + Par[8] * Taste + Par[9] * Affordability + Par[10] * Healthiness - Par[11] * C_custj + 0.02 * S_actual_all[i - 2];
